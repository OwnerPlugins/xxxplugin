...
try:
    folder_path = sum([sum(map(lambda fname: os.path.getsize(os.path.join(
        folder_path, fname)), files)) for folder_p, folders, files in os.walk(folder_path)])
...
def returnIMDB(text_clear):
    ...
    for plugin_name, (module_name, func_name) in plugins.items():
        plugin_path = resolveFilename(
            SCOPE_PLUGINS, "Extensions/{}".format(plugin_name))
        if os.path.exists(plugin_path):
            try:
                plugin_module = __import__(
                    "Plugins.Extensions.{}.plugin".format(module_name),
                    fromlist=[func_name])
                plugin_function = getattr(plugin_module, func_name)
                _session.open(
                    plugin_function,
                    text,
                    0 if func_name == "tmdbScreen" else None)
                return True
...
def threadGetPage(
        ...
        *args,
        **kwargs):
    print('[xxxplugin][threadGetPage] url, file, key, args, kwargs',
                ...
                try:
                    ...
                    try:
                        if imagew < size[0]:
                            ...
                            try:
                                im = im.resize(
                                    (int(imagew * ratio), int(imageh * ratio)), Image.LANCZOS)
                            except BaseException:
                                im = im.resize(
    ...
    def __init__(self, session):
        ...
        self["actions"] = ActionMap(
            ["WizardActions", "InputActions", "ColorActions",
                "ButtonSetupActions", "DirectionActions"],
            {
                "ok": self.okClicked,
                "back": self.close,
    ...
    def download_m3u(self, result):
        if result:
            if 'm3u8' not in self.urlm3u:
                ...
                # Rimpiazzo tutti i caratteri indesiderati
                fileTitle = re.sub(
                    r'[<>:"/\\|?*\[\]#()+\'!&]', '_', self.namem3u)
                # Rimpiazzo gli spazi con underscore
                fileTitle = re.sub(r'\s+', '_', fileTitle)
                # Riduco gli underscore consecutivi a uno solo
    ...
    def okClicked(self):
        ...
        if idx == 1:
            ...
            try:
                ...
                pass
            header = ''
            cmd = 'python "/usr/lib/enigma2/python/Plugins/Extensions/xxxplugin/lib/hlsclient.py" "' + \
                self.url + '" "1" "' + header + '" + &'

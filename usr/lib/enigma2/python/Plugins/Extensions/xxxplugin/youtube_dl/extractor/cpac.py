...
class CPACIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        video_url = try_get(
            content,
            lambda x: x['page']['details']['videoUrl'],
            compat_str)
        formats = []
        if video_url:
            content = content['page']
            title = str_or_none(
                content['details']['title_%s_t' % (url_lang, )])
        ...
        category = str_or_none(
            content['details']['category_%s_t' % (url_lang, )])
...
class CPACPlaylistIE(InfoExtractor):
    ...
    _TESTS = [{'url': 'https://www.cpac.ca/program?id=6',
               'info_dict': {'id': 'id=6',
              ...
              {'url': 'https://www.cpac.ca/search?key=hudson&type=all&order=desc',
               'info_dict': {'id': 'key=hudson',
                             'title': 'hudson',
                             },
               'playlist_count': 22,
               },
              {'url': 'https://www.cpac.ca/search?programId=50',
               'info_dict': {'id': 'programId=50',
                             'title': '50',
                             },
               'playlist_count': 9,
               },
    ...
    def _real_extract(self, url):
        ...
        url_lang = 'fr' if any(
            x in url for x in (
                '/emission?',
                '/rechercher?')) else 'en'
        pl_type, list_type = ('program', 'itemList') if any(x in url for x in (
            '/program?', '/emission?')) else ('search', 'searchResult')
        api_url = (
        ...
        total_pages = int_or_none(
            try_get(
                content,
                lambda x: x['page'][list_type]['totalPages']),
            default=1)
        for page in range(1, total_pages + 1):
            ...
            for item in try_get(
                    content,
                    lambda x: x['page'][list_type]['item'],
                    list) or []:
                episode_url = urljoin(url, try_get(
                    item, lambda x: x['url_%s_s' % (url_lang, )]))
                if episode_url:

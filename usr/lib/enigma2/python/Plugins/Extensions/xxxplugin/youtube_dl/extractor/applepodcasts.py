...
class ApplePodcastsIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        episode_data = try_get(
            amp_data,
            lambda a: next(
                x for x in a if x['type'] == 'podcast-episodes' and x['id'] == episode_id),
            dict)
        if not episode_data:
            # try pre 2021-11 page type: TODO: consider deleting if no longer
            # used
        ...
        series = series or clean_html(
            get_element_by_class(
                'podcast-header__identity', webpage))

...
def _js_add(a, b):
    if not (
        isinstance(
            a,
            compat_basestring) or isinstance(
            b,
            compat_basestring)):
...
_OPERATOR_RE = '|'.join(
    map(lambda x: re.escape(x[0]), _OPERATORS + _LOG_OPERATORS))
        ...
        def interpret_statement(
            ...
            if cls.ENABLED and stmt.strip():
                if should_ret or repr(ret) != stmt:
                    cls.write(['->', '=>'][bool(should_ret)],
                              repr(ret), '<-|', stmt, level=allow_recursion)
...
class JSInterpreter(object):
    ...
    class JS_RegExp(object):
        ...
        def __instantiate(self):
            ...
            # Thx:
            # https://stackoverflow.com/questions/44773522/setattr-on-python2-sre-sre-pattern
        ...
        def __getattr__(self, name):
            ...
            raise AttributeError(
                '{0} has no attribute named {1}'.format(
                    self, name))
    ...
    def _named_object(self, namespace, obj):
        ...
        if callable(obj) and not isinstance(obj, function_with_repr):
            obj = function_with_repr(
                obj, 'F<%s>' %
                (self.__named_object_counter, ))
        namespace[name] = obj
        return name
        ...
        for idx, char in enumerate(expr):
            ...
            if not in_quote:
                if char == '/' and expr[idx:idx + 2] == '/*':
                    ...
                    skip_txt = [
                        idx, idx + skip_txt + 1] if skip_txt >= 2 else None
                    if skip_txt:
                        continue
            ...
            after_op = not in_quote and (
                char in cls.OP_CHARS or paren_delta > 0 or (
                    after_op and char.isspace()))
    ...
    def _separate_at_paren(cls, expr, delim=None):
        ...
        if len(separated) < 2:
            raise cls.Exception(
                'No terminating paren {delim} in {expr!r:.5500}'.format(
                    **locals()))
    ...
    def _operator(
        ...
        opfunc = op and next(
            (v for k, v in self._all_operators() if k == op), None)
        if not opfunc:
            return right_val
        ...
        try:
            ...
            raise self.Exception(
                'Failed to evaluate {left_val!r:.50} {op} {right_val!r:.50}'.format(
                    **locals()), expr, cause=e)
    ...
    def _index(self, obj, idx, allow_undefined=True):
        if idx == 'length' and isinstance(obj, list):
            return len(obj)
        try:
            return obj[int(idx)] if isinstance(
                obj, list) else obj[compat_str(idx)]
        except (TypeError, KeyError, IndexError) as e:
            if allow_undefined:
                # when is not allowed?
                return JS_Undefined
            raise self.Exception(
                'Cannot get index {idx!r:.100}'.format(
                    **locals()), expr=repr(obj), cause=e)
    ...
    def handle_operators(self, expr, local_vars, allow_recursion):
        ...
        for op, _ in self._all_operators():
            # hackety: </> have higher priority than <</>>, but don't confuse
            # them
            ...
            if op in ('+', '-'):
                ...
                if op == '-' and undone % 2 != 0:
                    ...
                    while len(separated) > 1 and set(
                            separated[-1]) <= self.OP_CHARS:
                        right_expr = separated.pop() + right_expr
                    if separated[-1][-1:] in self.OP_CHARS:
                        right_expr = separated.pop() + right_expr
                # hanging op at end of left => unary + (strip) or - (push
                # right)
            ...
            return self._operator(
                op, left_val, right_expr, expr, local_vars, allow_recursion), True
    ...
    def interpret_statement(self, stmt, local_vars, allow_recursion=100):
        ...
        if expr[0] in _QUOTES:
            ...
            if expr[0] == '/':
                ...
                # , strict=True))
                inner = json.loads(js_to_json(inner + expr[0]))
            if not outer:
        ...
        if not new_kw:
            for klass, konstr in (
                ('Date', lambda x: int(
                    unified_timestamp(
                        x, False) * 1000)), ('RegExp', self.JS_RegExp), ('Error', self.Exception)):
                if not obj.startswith(klass + '('):
                    continue
                left, right = self._separate_at_paren(obj[len(klass):])
                argvals = self.interpret_iter(
                    left, local_vars, allow_recursion)
                expr = konstr(*argvals)
                if expr is None:
                    raise self.Exception(
                        'Failed to parse {klass} {left!r:.100}'.format(
                            **locals()), expr=expr)
                expr = self._dump(expr, local_vars) + right
                break
            else:
                raise self.Exception(
                    'Unsupported object {obj:.100}'.format(
                        **locals()), expr=expr)
        ...
        for op, _ in _UNARY_OPERATORS_X:
            ...
            if not operand or operand[0] != ' ':
                continue
            op_result = self.handle_operators(
                expr, local_vars, allow_recursion)
            if op_result:
            ...
            sub_expressions = [
                list(
                    self._separate(
                        sub_expr.strip(),
                        ':',
                        1)) for sub_expr in self._separate(inner)]
        ...
        if expr.startswith('('):
            m = re.match(
                r'\((?P<d>[a-z])%(?P<e>[a-z])\.length\+(?P=e)\.length\)%(?P=e)\.length',
                expr)
            if m:
                # short-cut eval of frequently used
                # `(d%e.length+e.length)%e.length`, worth ~6% on `pytest -k
                # test_nsig`
                outer = None
                inner, should_abort = self._offset_e_by_d(
                    m.group('d'), m.group('e'), local_vars)
            else:
                inner, outer = self._separate_at_paren(expr)
                inner, should_abort = self.interpret_statement(
                    inner, local_vars, allow_recursion)
        ...
        if md.get('if'):
            ...
            if expr.startswith('{'):
                ...
                if_expr, expr = self._separate_at_paren(
                    ' %s;' % (expr,), delim=';')
            else_expr = None
            m = re.match(r'else\s*(?P<block>\{)?', expr)
            if m:
                if m.group('block'):
                    else_expr, expr = self._separate_at_paren(
                        expr[m.end() - 1:])
                else:
                    # handle subset ... else if (...) {...} else ...
                    # TODO: make interpret_statement do this properly, if
                    # possible
                    exprs = list(self._separate(
                        expr[m.end():], delim='}', max_split=2))
                    if len(exprs) > 1:
                        if re.match(
                                r'\s*if\s*\(',
                                exprs[0]) and re.match(
                                r'\s*else\b',
                                exprs[1]):
                            else_expr = exprs[0] + '}' + exprs[1]
                            expr = (
                                exprs[2] + '}') if len(exprs) == 3 else None
                        else:
                            else_expr = exprs[0]
                            exprs.append('')
                            expr = '}'.join(exprs[1:])
            ...
            try:
                ...
                # XXX: This works for now, but makes debugging future issues
                # very hard
                err = e

            pending = (None, False)
            m = re.match(
                r'catch\s*(?P<err>\(\s*{_NAME_RE}\s*\))?\{{'.format(**globals()), expr)
            if m:
            ...
            init_or_cond, remaining = self._separate_at_paren(
                expr[m.end() - 1:])
            ...
            for default in (False, True):
                ...
                for item in items:
                    ...
                    if default:
                        ...
                        matched = (
                            case != 'default' and switch_val == self.interpret_expression(
        ...
        if md.get('assign'):
            ...
            if not m.group('out_idx'):
                ...
                raise self.Exception(
                    'Cannot index undefined variable ' +
                    m.group('out'),
                    expr=expr)
            ...
            for i, idx in enumerate(indexes, 1):
                idx = self.interpret_expression(
                    idx, local_vars, allow_recursion)
                if i < len(indexes):
            ...
            if should_return == 'return':
                ...
                return (
                    ret,
                    True) if ret != local_vars['_ytdl_do_not_return'] else (
                    ret,
                    False)
            else:
                return ret, should_return
        ...
        if md.get('attribute'):
                    ...
                    if new_member == 'prototype':
                        ...
                        assertion(
                            isinstance(
                                argvals[0],
                                obj),
                            'must bind to type {0}'.format(obj))
                ...
                if member == 'split':
                    ...
                    return (
                        obj.split(
                            argvals[0],
                            limit -
                            1) if argvals[0] and argvals[0] != JS_Undefined else list(obj)[
                            :limit or None])
                elif member == 'join':
                    assertion(
                        isinstance(
                    ...
                    assertion(isinstance(obj, (list, compat_str)),
                    ...
                    if index < 0:
                        index += len(obj)
                    res = [
                        obj.pop(index) for _ in range(
                            index, min(
                                index + how_many, len(obj)))]
                    ...
                    assertion(not argvals, 'does not take any arguments')
                    return obj.pop(
                        0 if member == 'shift' else -
                        1) if len(obj) > 0 else JS_Undefined
                elif member == 'unshift':
                    assertion(
                        isinstance(
                            obj,
                            list),
                        'must be applied on a list')
                    # not enforced: assertion(argvals, 'takes one or more
                    # arguments')
                    obj[0:0] = argvals
                    return len(obj)
                elif member == 'push':
                    ...
                    return [f((item,
                               ...
                               obj),
                              {'this': this},
                              allow_recursion) for idx,
                            item in enumerate(obj)]
                elif member == 'indexOf':
                    assertion(argvals, 'takes one or more arguments')
                    assertion(len(argvals) <= 2, 'takes at most 2 arguments')
                    ...
                    assertion(isinstance(obj, compat_str),
                              'must be applied on a string')
                    # assertion(len(argvals) == 1, 'takes exactly one
                    # argument') # but not enforced
                    idx = argvals[0] if len(argvals) > 0 and isinstance(
                        argvals[0], int) else 0
                    if idx >= len(obj):
                        return None
                    return ord(obj[idx])
            ...
            argvals = [
                self.interpret_expression(
                    v,
                    local_vars,
                    allow_recursion) for v in self._separate(
                    m.group('args'))]
        ...
        raise self.Exception('Unsupported JS expression ' +
                             (expr[:40] if expr != stmt else ''), expr=stmt)
    ...
    def extract_function_code(self, funcname):
        ...
        if func_m is None:
            raise self.Exception(
                'Could not find JS function "{funcname}"'.format(
                    **locals()))
        code, _ = self._separate_at_paren(
            func_m.group('code'))  # refine the match
        return self.build_arglist(func_m.group('args')), code

    def extract_function(self, funcname):
        return function_with_repr(
            self.extract_function_from_code(
                *self.extract_function_code(funcname)), 'F<%s>' %
            (funcname,))

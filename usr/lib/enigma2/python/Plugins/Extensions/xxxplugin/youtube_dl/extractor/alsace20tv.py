...
class Alsace20TVIE(InfoExtractor):
    ...
    def _extract_video(self, video_id, url=None):
        info = self._download_json(
            'https://www.alsace20.tv/visionneuse/visio_v9_js.php?key=%s&habillage=0&mode=html' %
        ...
        webpage = (
            url and self._download_webpage(
                url,
                video_id,
                fatal=False)) or ''
        ...
        upload_date = unified_strdate(
            '20%s-%s-%s' % (upload_date[:2], upload_date[2:4], upload_date[4:])) if upload_date else None
        return {
            'id': video_id,
            'title': title,
            'formats': formats,

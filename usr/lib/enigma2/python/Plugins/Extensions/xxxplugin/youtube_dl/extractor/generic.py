...
class GenericIE(InfoExtractor):
    ...
    _TESTS = [
        ...
        # SMIL from
        # http://adventure.howstuffworks.com/5266-cool-jobs-iditarod-musher-video.htm
        {
        ...
        # XSPF playlist from
        # http://www.telegraaf.nl/tv/nieuws/binnenland/24353229/__Tikibad_ontruimd_wegens_brand__.html
        {
        ...
        # ooyala video embedded with
        # http://player.ooyala.com/static/v4/production/latest/core.min.js
        {
        ...
        # ClipYou (EaglePlatform) embed (custom URL)
        {
            ...
            # Not checking MD5 as sometimes the direct HTTP link results in 404
            # and HLS is used
            'info_dict': {
                'id': '12820',
                'ext': 'mp4',
    ...
    def report_following_redirect(self, new_url):
        ...
        self._downloader.to_screen(
            '[redirect] Following redirect to %s' %
            new_url)
    ...
    def _extract_kvs(self, url, webpage, video_id):
        ...
        flashvars = self._parse_json(
            flashvars, video_id, transform_source=js_to_json)
    ...
    def _real_extract(self, url):
        ...
        if not self._downloader.params.get(
                'test', False) and not is_intentional:
            force = self._downloader.params.get(
        ...
        if nexx_urls:
            return self.playlist_from_matches(
                nexx_urls, video_id, video_title, ie=NexxIE.ie_key())

        # Look for Nexx iFrame embeds
        nexx_embed_urls = NexxEmbedIE._extract_urls(webpage)
        if nexx_embed_urls:
            return self.playlist_from_matches(
                nexx_embed_urls, video_id, video_title, ie=NexxEmbedIE.ie_key())

        # Look for ThePlatform embeds
        tp_urls = ThePlatformIE._extract_urls(webpage)
        if tp_urls:
            return self.playlist_from_matches(
                tp_urls, video_id, video_title, ie='ThePlatform')

        arc_urls = ArcPublishingIE._extract_urls(webpage)
        if arc_urls:
            return self.playlist_from_matches(
                arc_urls, video_id, video_title, ie=ArcPublishingIE.ie_key())
        ...
        if matches:
            return self.playlist_from_matches(
                matches, video_id, video_title, ie='RtlNl')

        vimeo_urls = VimeoIE._extract_urls(url, webpage)
        if vimeo_urls:
            return self.playlist_from_matches(
                vimeo_urls, video_id, video_title, ie=VimeoIE.ie_key())
        ...
        if m:
            ...
            if playlists:
                return self.playlist_from_matches(
                    ...
                    video_id,
                    video_title,
                    lambda p: '//dailymotion.com/playlist/%s' %
                    p)
        ...
        if wistia_urls:
            playlist = self.playlist_from_matches(
                wistia_urls, video_id, video_title, ie=WistiaIE.ie_key())
        ...
        mobj = re.search(
            r'<meta property="og:url"[^>]*?content="(.*?bandcamp\.com.*?)"',
            webpage)
        if mobj is not None:
        ...
        mobj = (
            re.search(
                ...
                r'OO\.Player\.create\([\'"].*?[\'"],\s*[\'"](?P<ec>.{32})[\'"]',
                webpage) or re.search(
                r'OO\.Player\.create\.apply\(\s*OO\.Player\s*,\s*op\(\s*\[\s*[\'"][^\'"]*[\'"]\s*,\s*[\'"](?P<ec>.{32})[\'"]',
                webpage) or re.search(
                    r'SBN\.VideoLinkset\.ooyala\([\'"](?P<ec>.{32})[\'"]\)',
                    webpage) or re.search(
                        r'data-ooyala-video-id\s*=\s*[\'"](?P<ec>.{32})[\'"]',
                webpage))
        if mobj is not None:
        ...
        if mobj is not None:
            ...
            if embeds:
                return self.playlist_from_matches(
                    embeds, video_id, video_title, getter=lambda v: OoyalaIE._url_for_embed_code(
                        smuggle_url(
                            v['provider_video_id'], {
                                'domain': url})), ie='Ooyala')

        # Look for Aparat videos
        mobj = re.search(
            r'<iframe .*?src="(http://www\.aparat\.com/video/[^"]+)"',
            webpage)
        if mobj is not None:
        ...
        mobj = re.search(
            r'<iframe .*?src="(http://mpora\.(?:com|de)/videos/[^"]+)"',
            webpage)
        if mobj is not None:
        ...
        if facebook_urls:
            return self.playlist_from_matches(
                facebook_urls, video_id, video_title)
        ...
        if sibnet_urls:
            return self.playlist_from_matches(
                sibnet_urls, video_id, video_title)
        ...
        matches = re.findall(
            r'<iframe[^>]+?src="(https?://(?:www\.)?funnyordie\.com/embed/[^"]+)"',
            webpage)
        if matches:
            return self.playlist_from_matches(
                matches,
        ...
        if matches:
            return self.playlist_from_matches(
                matches, video_id, video_title, ie='BBCCoUk')
        ...
        if sportbox_urls:
            return self.playlist_from_matches(
                sportbox_urls, video_id, video_title, ie=SportBoxIE.ie_key())

        # Look for embedded XHamster player
        xhamster_urls = XHamsterEmbedIE._extract_urls(webpage)
        if xhamster_urls:
            return self.playlist_from_matches(
                xhamster_urls, video_id, video_title, ie='XHamsterEmbed')

        # Look for embedded TNAFlixNetwork player
        tnaflix_urls = TNAFlixNetworkEmbedIE._extract_urls(webpage)
        if tnaflix_urls:
            return self.playlist_from_matches(
                tnaflix_urls, video_id, video_title, ie=TNAFlixNetworkEmbedIE.ie_key())

        # Look for embedded PornHub player
        pornhub_urls = PornHubIE._extract_urls(webpage)
        if pornhub_urls:
            return self.playlist_from_matches(
                pornhub_urls, video_id, video_title, ie=PornHubIE.ie_key())

        # Look for embedded DrTuber player
        drtuber_urls = DrTuberIE._extract_urls(webpage)
        if drtuber_urls:
            return self.playlist_from_matches(
                drtuber_urls, video_id, video_title, ie=DrTuberIE.ie_key())

        # Look for embedded RedTube player
        redtube_urls = RedTubeIE._extract_urls(webpage)
        if redtube_urls:
            return self.playlist_from_matches(
                redtube_urls, video_id, video_title, ie=RedTubeIE.ie_key())

        # Look for embedded Tube8 player
        tube8_urls = Tube8IE._extract_urls(webpage)
        if tube8_urls:
            return self.playlist_from_matches(
                tube8_urls, video_id, video_title, ie=Tube8IE.ie_key())

        # Look for embedded Mofosex player
        mofosex_urls = MofosexEmbedIE._extract_urls(webpage)
        if mofosex_urls:
            return self.playlist_from_matches(
                mofosex_urls, video_id, video_title, ie=MofosexEmbedIE.ie_key())

        # Look for embedded Spankwire player
        spankwire_urls = SpankwireIE._extract_urls(webpage)
        if spankwire_urls:
            return self.playlist_from_matches(
                spankwire_urls, video_id, video_title, ie=SpankwireIE.ie_key())

        # Look for embedded YouPorn player
        youporn_urls = YouPornIE._extract_urls(webpage)
        if youporn_urls:
            return self.playlist_from_matches(
                youporn_urls, video_id, video_title, ie=YouPornIE.ie_key())
        ...
        if soundcloud_urls:
            return self.playlist_from_matches(
                soundcloud_urls, video_id, video_title, getter=unescapeHTML)

        # Look for tunein player
        tunein_urls = TuneInBaseIE._extract_urls(webpage)
        if tunein_urls:
            return self.playlist_from_matches(
                tunein_urls, video_id, video_title)
        ...
        mobj = re.search(
            r'<(?:iframe|script)[^>]+?src=(["\'])(?P<url>%s)\1' %
            CondeNastIE.EMBED_URL, webpage)
        if mobj is not None:
            return self.url_result(
        ...
        # Look for UDN embeds
        mobj = re.search(
            r'<iframe[^>]+src="(?:https?:)?(?P<url>%s)"' %
            UDNEmbedIE._PROTOCOL_RELATIVE_VALID_URL, webpage)
        if mobj is not None:
            return self.url_result(
        ...
        if jwplatform_urls:
            return self.playlist_from_matches(
                jwplatform_urls, video_id, video_title, ie=JWPlatformIE.ie_key())

        # Look for Digiteka embeds
        digiteka_url = DigitekaIE._extract_url(webpage)
        if digiteka_url:
            return self.url_result(
                self._proto_relative_url(digiteka_url),
                DigitekaIE.ie_key())
        ...
        if dbtv_urls:
            return self.playlist_from_matches(
                dbtv_urls, video_id, video_title, ie=DBTVIE.ie_key())

        # Look for Videa embeds
        videa_urls = VideaIE._extract_urls(webpage)
        if videa_urls:
            return self.playlist_from_matches(
                videa_urls, video_id, video_title, ie=VideaIE.ie_key())
        ...
        sharevideos_urls = [
            sharevideos_mobj.group('url') for sharevideos_mobj in re.finditer(
        ...
        found = self._search_regex(
            (r'<script\b[^>]+?\bsrc\s*=\s*(["\'])https?://(?:\S+?/)+kt_player\.js\?v=(?P<ver>\d+(?:\.\d+)+)\1[^>]*>',
             # kt_player('kt_player',
             # 'https://i.shoosh.co/player/kt_player.swf?v=5.5.1', ...
        ...
        if found:
            ...
            if found.split('.')[0] not in ('4', '5', '6'):
                self.report_warning(
                    'Untested major version (%s) in player engine - download may fail.' %
                    (found, ))
            return merge_dicts(
        ...
        def check_video(vurl):
            ...
            return '.' in vpath and vext not in (
                'swf', 'png', 'jpg', 'srt', 'sbv', 'sub', 'vtt', 'ttml', 'js', 'xml')
        ...
        if not found:
            ...
            # try to align them (eg.: statigr.am)
        ...
        if not found:
            ...
            if found:
                new_url = compat_urlparse.urljoin(
        ...
        for video_url in orderedSet(found):
            ...
            if ext == 'smil':
                entry_info_dict['formats'] = self._extract_smil_formats(
                    video_url, video_id)
            elif ext == 'xspf':
                return self.playlist_result(
                    self._extract_xspf_playlist(
                        video_url, video_id), video_id)
            elif ext == 'm3u8':
                entry_info_dict['formats'] = self._extract_m3u8_formats(
                    video_url, video_id, ext='mp4')
            elif ext == 'mpd':
                entry_info_dict['formats'] = self._extract_mpd_formats(
                    video_url, video_id)
            elif ext == 'f4m':

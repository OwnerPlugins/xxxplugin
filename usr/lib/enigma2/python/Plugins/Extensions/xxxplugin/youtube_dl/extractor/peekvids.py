    ...
    def _real_extract(self, url):
        ...
        if '>Rate Limit Exceeded' in webpage:
            raise ExtractorError(
                ...
                (self.IE_NAME,
                 video_id,
                 'You are suspected as a bot. Wait, or pass the captcha test on the site and provide --cookies.'),
                expected=True)
        ...
        info['title'] = re.sub(
            r'\s*[,-][^,-]+$',
            '',
            info.get('title') or title) or self._generic_title(url)

        def cat_tags(name, html):
            l = self._html_search_regex(
                r'(?s)<span\b[^>]*>\s*%s\s*:\s*</span>(.+?)</li>' %
                (re.escape(name), ), html, name, default='')
...
class PlayVidsIE(PeekVidsIE):
    ...
    _TESTS = [{'url': 'https://www.playvids.com/U3pBrYhsjXM/pc/dane-jones-cute-redhead-with-perfect-tits-with-mini-vamp',
               'md5': '2f12e50213dd65f142175da633c4564c',
               'info_dict': {'id': '1978030',
